...
class TaskList extends Component {
    ...
    onChange = (event) => {
        ...
        var filter = {
            filterName: (name === 'filterName') ? value : this.props.filterTable.filterName,
            filterStatus: (name === 'filterStatus') ? value : this.props.filterTable.filterStatus
        };
    ...
    render() {
        ...
        if(filterTable){
			...
			tasks = tasks.filter((task) => {
				if(filterTable.filterStatus === 0){
					return task.status === false;
				}else if(filterTable.filterStatus === 1){
